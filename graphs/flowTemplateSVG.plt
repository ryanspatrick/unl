set terminal svg size 1920,1080 dynamic enhanced font "Courier,18" linewidth 3
...
plot 'FILE.nodes.csv' using (nX($1+1,1)):(nY($1+1,1)):(sprintf( "%c" , $1 + 65 )) with labels notitle,\
     ...
     for [e in edges] e using (startX($1,$2)):(startY($1,$2)):(dx($1,$2)):(dy($1,$2)) with vectors filled title columnhead(3)